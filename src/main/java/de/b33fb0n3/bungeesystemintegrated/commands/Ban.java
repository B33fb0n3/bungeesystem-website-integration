    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender.hasPermission("bungeecord.ban") || sender.hasPermission("bungeecord.*")) {
            if (args.length == 2 || args.length == 3) {
                ...
                de.b33fb0n3.bungeesystemintegrated.utils.Ban currentBan = new de.b33fb0n3.bungeesystemintegrated.utils.Ban(ptUUID, null, Bungeesystem.getPlugin().getDataSource(), Bungeesystem.settings, Bungeesystem.standardBans);
                currentBan.isBanned().whenComplete((result, ex) -> {
                    ...
                    currentBan.getBanCount(grund, true).whenComplete((banCountResult, exception) -> {
                        ...
                        new de.b33fb0n3.bungeesystemintegrated.utils.Ban(ptUUID, sender.getName(), grund, System.currentTimeMillis(), unban, permaint, banint, target != null ? target.getSocketAddress().toString() : "NULL", beweis, Bungeesystem.getPlugin().getDataSource(), Bungeesystem.settings, Bungeesystem.standardBans);
                    });
                });
            } else {
